<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta content="width=device-width, initial-scale=1.0" name="viewport">

    <title>Ticket - Tickers</title>
    <meta content="" name="description">
    <meta content="" name="keywords">

    <!-- Favicons -->
    <link href="assets/img/logo.jpg" rel="icon">
    <link href="assets/img/logo.jpg" rel="apple-touch-icon">

    <!-- Google Fonts -->
    <link href="https://fonts.gstatic.com" rel="preconnect">
    <link
        href="https://fonts.googleapis.com/css?family=Open+Sans:300,300i,400,400i,600,600i,700,700i|Nunito:300,300i,400,400i,600,600i,700,700i|Poppins:300,300i,400,400i,500,500i,600,600i,700,700i"
        rel="stylesheet">

    <!-- Vendor CSS Files -->
    <link href="assets/vendor/bootstrap/css/bootstrap.min.css" rel="stylesheet">
    <link href="assets/vendor/bootstrap-icons/bootstrap-icons.css" rel="stylesheet">
    <link href="assets/vendor/boxicons/css/boxicons.min.css" rel="stylesheet">
    <link href="assets/vendor/quill/quill.snow.css" rel="stylesheet">
    <link href="assets/vendor/quill/quill.bubble.css" rel="stylesheet">
    <link href="assets/vendor/remixicon/remixicon.css" rel="stylesheet">
    <link href="assets/vendor/simple-datatables/style.css" rel="stylesheet">

    <!-- Template Main CSS File -->
    <link href="assets/css/style.css" rel="stylesheet">

</head>

<body>

    <!-- ======= Header ======= -->
    <header id="header" class="header fixed-top d-flex align-items-center">

        <div>
            <a href="#" class="logo d-flex">
                <img src="assets/img/logo.jpg" alt="">
                <span class="d-none d-lg-block">Tickers</span>
            </a>
        </div><!-- End Logo -->

        <nav class="header-nav ms-auto">
            <ul class="d-flex align-items-center">

                <!-- Shopping Cart start -->
                <div class="shopping-cart" x-data="$store.cart">
                    <template x-for="(item, index) in $store.cart.items" :key="index">
                        <div class="cart-item">
                            <img :src="item . img" alt="tikets" class="img-fluid">
                            <div class="item-detail">
                                <h3 x-text="item.name"></h3>
                                <div class="item-price">
                                    <span x-text="item.price"></span> &times;
                                    <button @click="$store.cart.remove(index)">&minus;</button>
                                    <span x-text="item.quantity"></span>
                                    <button @click="$store.cart.add(item)">&plus;</button>=
                                    <span x-text="item.total"></span>
                                </div>
                            </div>
                        </div>
                    </template>
                    <div class="cart-total" x-show="$store.cart.items.length">
                        <div>Total : <span x-text="`Rp. ${$store.cart.total}`"></span></div>
                        <div>Items : <span x-text="$store.cart.quantity"></span></div><br>
                    </div>
                    <div class="form-container" x-show="$store.cart.items.length">
                        <form action="" id="checkoutForm">
                            <input type="hidden" name="items" :value="JSON . stringify($store . cart . items)">
                            <input type="hidden" name="total" :value="$store . cart . total">
                            <h4>Customer Detail</h4>

                            <label for="nama">
                                <span>Nama</span>
                                <input type="text" name="nama" id="nama">
                            </label>

                            <label for="telpon">
                                <span>Telpon</span>
                                <input type="number" name="telpon" id="telpon" autocomplete="off">
                            </label>
                            <button class="checkout-button" type="submit" id="checkout-button">Checkout</button>
                        </form>
                    </div>
                </div>
                <!-- Shopping Cart end -->


                <!-- button to open cart -->
                <div class="navbar-extra">
                    <a href="#" id="shopping-cart-button">
                        <i class="bi bi-cart fs-3"></i>
                        <span class="quantity-badge" x-show="$store.cart.quantity" x-text="$store.cart.quantity"></span>
                    </a>
                </div>

                <li class="nav-item dropdown pe-3 ms-2">


                    <a class="nav-link nav-profile d-flex align-items-center pe-0" href="#" data-bs-toggle="dropdown">
                        <img src="assets/img/profil-admin1.jpg" alt="Profile" class="rounded-circle">
                        <span class="d-none d-md-block dropdown-toggle ps-2">Salman King</span>
                    </a><!-- End Profile Iamge Icon -->

                    <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow profile">
                        <li class="dropdown-header">
                            <h6>Salman King</h6>
                            <span>Full Stack</span>
                        </li>
                        <li>
                            <hr class="dropdown-divider">
                        </li>

                        <li>
                            <a class="dropdown-item d-flex align-items-center" href="#">
                                <i class="bi bi-person"></i>
                                <span>My Profile</span>
                            </a>
                        </li>
                        <li>
                            <hr class="dropdown-divider">
                        </li>

                        <li>
                            <a class="dropdown-item d-flex align-items-center" href="#">
                                <i class="bi bi-gear"></i>
                                <span>Account Settings</span>
                            </a>
                        </li>
                        <li>
                            <hr class="dropdown-divider">
                        </li>

                        <li>
                            <hr class="dropdown-divider">
                        </li>

                        <li>
                            <a class="dropdown-item d-flex align-items-center" href="#">
                                <i class="bi bi-box-arrow-right"></i>
                                <span>Sign Out</span>
                            </a>
                        </li>

                    </ul><!-- End Profile Dropdown Items -->
                </li><!-- End Profile Nav -->

            </ul>
        </nav><!-- End Icons Navigation -->

    </header><!-- End Header -->

    <!-- ======= Sidebar ======= -->
    <aside id="sidebar" class="sidebar">

        <ul class="sidebar-nav" id="sidebar-nav">

            <li class="nav-item">
                <a class="nav-link collapsed" href="dashboard">
                    <i class="bi bi-grid"></i>
                    <span>Dashboard</span>
                </a>
            </li><!-- End Dashboard Nav -->

            <li class="nav-item">
                <a class="nav-link" data-bs-target="#components-nav" data-bs-toggle="collapse" href="#">
                    <i class="bi bi-ticket-perforated-fill"></i><span>Ticket</span><i
                        class="bi bi-chevron-down ms-auto"></i>
                </a>
                <ul id="components-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
                    <li>
                        <a href="buyTicket">
                            <i class="bi bi-circle"></i><span>Buy Ticket</span>
                        </a>
                    </li>
                    <li>
                        <a href="myTicket">
                            <i class="bi bi-circle"></i><span>My Ticket</span>
                        </a>
                    </li>
                </ul>
            </li><!-- End Components Nav -->

            <li class="nav-item">
                <a class="nav-link collapsed" data-bs-target="#forms-nav" data-bs-toggle="collapse" href="#">
                    <i class="bi bi-chat-right-dots-fill"></i><span>Live Chat</span><i
                        class="bi bi-chevron-down ms-auto"></i>
                </a>

            </li><!-- End Forms Nav -->

            <li class="nav-item">
                <a class="nav-link collapsed" data-bs-target="#tables-nav" data-bs-toggle="collapse" href="#">
                    <i class="bi bi-award-fill"></i><span>Reward</span><i class="bi bi-chevron-down ms-auto"></i>
                </a>
                <ul id="tables-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
                    <li>
                        <a href="#">
                            <i class="bi bi-circle"></i><span>Voucher</span>
                        </a>
                    </li>
                    <li>
                        <a href="#">
                            <i class="bi bi-circle"></i><span>My Voucher</span>
                        </a>
                    </li>
                </ul>
            </li><!-- End Tables Nav -->

            <li class="nav-item">
                <a class="nav-link collapsed" href="about">
                    <i class="bi bi-file-person"></i>
                    <span>About</span>
                </a>
            </li><!-- End About Nav -->




            <li class="nav-heading">More Information</li>

            <li class="nav-item">
                <a class="nav-link collapsed" href="#">
                    <i class="bi bi-person"></i>
                    <span>Account</span>
                </a>
            </li><!-- End Profile Page Nav -->

            <li class="nav-item">
                <a class="nav-link collapsed" href="#">
                    <i class="bi bi-box-arrow-in-right"></i>
                    <span>Quit</span>
                </a>
            </li><!-- End Login Page Nav -->

        </ul>

    </aside><!-- End Sidebar-->

    <main id="main" class="main">

        <h1>Ticket</h1>
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="#">Ticket</a></li>
                <li class="breadcrumb-item active">Informasi Ticket</li>

            </ol>


        </nav>


        </div><!-- End Page Title -->
        @foreach ($seminars as $seminar)

            <div class="container">

                <!-- Image -->
                <div class="row justify-content-center mb-4">
                    <div class="col-md-6 text-center">
                        <img src="\assets\img\Poster Sendikom 2024.png" class="img-fluid" alt="Event Image">
                    </div>
                </div>

                <!-- Judul Seminar -->
                @if (isset($seminar->nama_seminar))
                    <h2>{{ $seminar->nama_seminar }}</h2>
                @else
                    <h2>Judul Seminar Tidak Tersedia</h2>
                @endif

                <!-- Deskripsi Seminar -->
                @if (isset($seminar->deskripsi))
                    <p>{{ $seminar->deskripsi }}</p>
                @else
                    <p>Deskripsi Seminar Tidak Tersedia</p>
                @endif

                @if (isset($seminar->tempat))
                    <div class="info"> <i class="bi bi-pin-map-fill"></i> <span class="ms-2">{{$seminar->tempat}}</span>
                    </div>
                @endif

                @if (isset($seminar->tanggal))
                    <div class="info"> <i class="bi bi-calendar-event"></i> <span class="ms-2">{{$seminar->tanggal}}</span>
                    </div>
                @endif

                @if (isset($seminar->waktu))
                    <div class="info"> <i class="bi bi-clock-fill"></i> <span class="ms-2">{{$seminar->waktu}}</span> </div>
                @endif

                <!-- harga seminar -->
                @if (isset($seminar->harga))
                    <div class="info"> <i class="bi bi-tag-fill"></i> <span class="ms-2">{{$seminar->harga}}</span> </div>
                @endif

                <div class="actions" x-data="tikets">
                    <template x-for="(item, index) in items" :key="index">
                        <div>
                            <button @click.prevent="$store.cart.add(item)">
                                <i class="bi bi-cart-plus-fill"></i><span class="ms-2">Buy</span>
                            </button>
                            <button class="ms-2"><i class="bi bi-bookmark-fill"></i><span class="ms-2">Save</span></button>
                        </div>
                    </template>
                </div>

                <p style="text-align: center; margin-top: 10px;">Created By : FKIP MENGAJAR</p>
            </div>
            </div><!-- End Page Title -->
        @endforeach

    </main><!-- End #main -->

    <!-- ======= Footer ======= -->
    <footer id="footer" class="footer">
        <div class="copyright">
            &copy; Copyright <strong><span>Tickers</span></strong>. All Rights Reserved
        </div>
        <div class="credits">
            Designed by <a href="about">Team Tickers</a>
        </div>
    </footer><!-- End Footer -->

    <a href="#" class="back-to-top d-flex align-items-center justify-content-center"><i
            class="bi bi-arrow-up-short"></i></a>

    <!-- Vendor JS Files -->
    <script src="assets/vendor/apexcharts/apexcharts.min.js"></script>
    <script src="assets/vendor/bootstrap/js/bootstrap.bundle.min.js"></script>
    <script src="assets/vendor/chart.js/chart.umd.js"></script>
    <script src="assets/vendor/echarts/echarts.min.js"></script>
    <script src="assets/vendor/quill/quill.js"></script>
    <script src="assets/vendor/simple-datatables/simple-datatables.js"></script>
    <script src="assets/vendor/tinymce/tinymce.min.js"></script>
    <script src="assets/vendor/php-email-form/validate.js"></script>

    <!-- Template Main JS File -->
    <script src="assets/js/main.js"></script>
    <script src="assets/js/app.js" async></script>

    <!-- Alpine JS -->
    <script defer src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js"></script>

</body>


</html>